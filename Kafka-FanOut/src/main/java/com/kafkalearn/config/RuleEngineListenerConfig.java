...
@Configuration
public class RuleEngineListenerConfig {
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, String> ruleEngineKafkaListenerContainerFactory(
            ConsumerFactory<String, String> consumerFactory, KafkaTemplate<String, String> kafkaTemplate) {

        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(
                kafkaTemplate,
                (record, ex) -> new TopicPartition(
                        KafkaTopic.DEVICE_ALERT_RULE_DLQ_TOPIC,
                        record.partition()));
